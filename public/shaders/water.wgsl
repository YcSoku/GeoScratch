...
fn fMain(fsInput: VertexOutput) -> @location(0) vec4f {
    ...
    let mask = 1.0 - textureSample(landMask, lsampler, offsetedUV).r;
    ...
    return vec4(result, mix(sin(diff * 3.141592653 / 2.0), 1.0, sin(diff * 3.141592653 / 2.0)));
    // return vec4(result, mix(diff, 1.0, diff));
    // return vec4(result, mix(diff, 1.0, diff));
}